...
fn command_lookup_preserves_original_table() {
    ...
    assert.eq(parser.cmd_to_index(), CmdWhich::Tmp);
    assert.eq(CmdWhich::Pwon.as_str(), "RON");
    assert.eq(CmdWhich::from_mnemonic("rof"), CmdWhich::Pwoff);
    assert.eq(CmdWhich::Pwoff.default_subchannel(), 28);
    assert.finish();
}
...
fn mixed_case_command_mnemonics_parse_like_pascal() {
    ...
    assert.eq(parser.cmd_which, CmdWhich::Dcv);
    assert.is_true(
        parser
        .serial_log
        .iter()
...
fn set_commands_check_limits_and_report_verbose_limit_errors() {
    ...
    assert.eq(parser.dc_volt, parser.u_max);
    assert.eq(parser.check_limit_err, super::Error::ParamErr);
    assert.is_true(parser.serial_log.iter().any(|line| line == "[PARERR]"));
    assert.finish();
}
...
fn successful_set_commands_apply_new_dac_level() {
    ...
    assert.eq(parser.check_limit_err, super::Error::NoErr);
    assert.eq(parser.dc_volt, 5.0);
    assert.eq(parser.dac_raw_uon, 11);
    assert.eq(parser.dac_raw_uoff, 11);
    assert.finish();
}
...
fn option_writes_reload_calibration_and_honor_settle_delay() {
    ...
    assert.eq(parser.u_max, 24.0);
    assert.eq(parser.option_array[OptionSlot::MaximumVoltage.index()], 24.0);
    assert.eq(parser.delay_log, vec![3]);
    assert.finish();
}
...
fn eeprom_scale_writes_recompute_lsb_calibration() {
    ...
    assert.eq(parser.dac_u_scales[0], 2.0);
    assert.is_true((parser.dac_lsb_u[0] - 0.00011444092).abs() < 0.00000001);
    assert.eq(parser.delay_log, vec![3]);
    assert.finish();
}
...
fn dsp_modify_writes_refresh_display_state() {
    ...
    assert.eq(parser.modify, super::Modify::Ripple);
...
fn local_parser_activity_drives_activity_led_low() {
    ...
    assert.eq(parser.activity_timer, 255);
    assert.is_true(parser.led_activity_low);
    assert.finish();
}
...
fn sub_channel_253_echoes_input_without_reply_prefix() {
    ...
    assert.eq(parser.serial_log, vec!["0:253?"]);
    assert.finish();
}
...
fn sub_channel_254_uses_pascal_reply_framing() {
    ...
    assert.eq(parser.serial_log, vec![format!("#0:254={}", super::VERS1_STR)]);
    assert.finish();
}
