...
impl CmdWhich {
    ...
    /// Maps the protocol-table position to its command identity, using `Err` for every out-of-range index so malformed input cannot dispatch a valid command.
    ...
    /// Returns the command's wire-table position; the enum discriminants deliberately match the ordered mnemonic and subchannel tables.
